...
- 会话恢复：浏览器保存 room-scoped session token，刷新后可通过 `/resume` 恢复同一玩家身份；WebSocket 断开后自动重连。
- 房主治理：大厅踢人、转交房主、玩家退出；踢人/退出会同步移出 WebSocket 和 LiveKit 语音房间。
...
当前 v2 已覆盖第 1-12 步的完整一局链路。P0 第 1-7 项已经补齐规则闭环；P1 第 8-12 项已经形成第一版朋友局线上体验，包含刷新恢复、WebSocket 自动重连和语音房间移出。后续仍可继续增强生产持久化、多端恢复和更完整的自动化浏览器验收。
